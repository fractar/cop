...
\begin{figure}[!h]
    \begin{minipage}[t]{6cm}
        \centering
        \includegraphics[width=7cm]{./pictures/densityx.png}
    \end{minipage}
    \begin{minipage}[t]{6cm}
        \centering
        \includegraphics[width=7cm]{./pictures/densityy.png}
    \end{minipage}
    \caption{Densités empiriques de X et de Y}
\end{figure}
...
\begin{figure}[!h]
\centering
\includegraphics[scale=0.4]{./pictures/Fdrx.png}
\caption{Fonctions de répartition empiriques}
\end{figure}

\begin{figure}[!h]
\centering
\includegraphics[scale=0.4]{./pictures/Fdry.png}
\caption{Fonctions de répartition empiriques}
\end{figure}

\newpage
En traçant les fonctions de répartitions empiriques et calibrées pour chaque marginale, on constate que la loi exponentielle n'est pas du tout adaptée. Il est difficile de déterminer graphiquement laquelle des lois restantes est la plus proche de la distribution empirique. Il est nécessaire de comparer les log-vraisemblances.
\\ \\
...
Pour la méthode de maximum de vraisemblance, on constate que la copule normale est la plus appropriée. Dans ce cas, la force maximale du vent à Saint Martin en Haut suit une loi de Gumbel de paramètres($\mu = , \beta= )$. La force maximale du vent à Echirolles suit une loi Gamma de paramètres $(a= , b=)$ et le paramètre de la copule a pour estimation $\alpha=0.523$.
...
Dans un premier temps, on estime les paramètres des marginales. Nous avons estimé que la force maximale du vent à Echirolles suivait une loi gamma (a=5.022,b=1.83). Celle de Saint Martin en Haut suit une loi de Gumbel $(\mu =4.968,\beta = 0.754)$. Reste maintenant à tester les cinq copules et à estimer leurs paramètres.
\\ \\
...
Pour la méthode IFM, on constate également que la copule normale est la plus appropriée. Le paramètre de cette copule a pour estimation $\hat{\alpha}=0.521$.
